...
export default function MemberHeader({ memberProfile }: MemberHeaderProps) {
  const navigate = useNavigate();
  const { memberId, memberName, logout } = useMemberAuth();
  ...
  const displayName = memberProfile?.name || memberName || 'Member';
          ...
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="relative h-8 w-8 md:h-10 md:w-10 rounded-full">
              <Avatar className="h-8 w-8 md:h-10 md:w-10">
                <AvatarImage
                  src="/assets/generated/default-avatar-transparent.dim_100x100.png"
                  alt={displayName}
                />
